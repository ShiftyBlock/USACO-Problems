...
public class race {
    ...
    public static void main(String[] args) throws IOException {
        boolean debug = !(true);
        FastScanner sc = new FastScanner("race.in", debug);
        FastWriter pw = new FastWriter("race.out", debug);
        int dist= sc.ni();
        int N= sc.ni();
        while(N-->0){
            int end= sc.ni();
            pw.println(ans(dist,end));
        }
    ...
    public static int ans(int tdist, int end){
        int[] prefix= new int[600000];
        ...
        int curpos=0;
        while(curpos<tdist){
            if((curspeed+1)<=end || prefix[curspeed+1]-prefix[end-1]<=tdist-curpos) curspeed++;
            else if(prefix[curspeed]-prefix[end-1]<=tdist-curpos) curspeed+=0;
            else curspeed--;
            curtime++;
            curpos+=curspeed;
        }
