...
public class snakes {
    ...
    public static void main(String[] args) throws IOException {
        boolean debug = !(true);
        FastScanner sc = new FastScanner("snakes.in", debug);
        FastWriter pw = new FastWriter("snakes.out", debug);
        int N= sc.ni();
        int K= sc.ni();
        ...
        for (int i = 0; i < N; i++) {
            for (int j = i; j <N ; j++) {
                if(j==i)maxarr[i][j]=arr[j];
                else maxarr[i][j]= Math.max(arr[j], maxarr[i][j-1]);
            }
        }
        for (int i = 1; i <N+1 ; i++) {
            for (int j = 1; j < K+1; j++) {
                int curm=0;
                for (int k = 0; k <i ; k++) {
                    dp[i][j]= Math.min(dp[i][j], dp[k][j-1]+(i-k)*maxarr[k][i-1]);
                }
            }
        }

        pw.println(dp[N][K]-totalsum);
