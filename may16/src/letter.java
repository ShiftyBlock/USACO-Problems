...
public class letter {
    ...
    public static void main(String[] args) throws IOException {
        FastScanner sc = new FastScanner("letter.in", true);
        FastWriter pw = new FastWriter("letter.out", true);
        String line= sc.next();
        int[] lower= new int[line.length()+1];
        int[] upper= new int[line.length()+1];
        int lowersum=0;
        for (int i = 0; i < line.length() ; i++) {
            lower[i]=lowersum;
            if (Character.isLowerCase(line.charAt(i))) lowersum++;
        }
        lower[line.length()]=lowersum;
        int uppersum=0;
        for (int i = line.length()-1; i >=0 ; i--) {
            upper[i]=uppersum;
            if (Character.isUpperCase(line.charAt(i))) uppersum++;
        }
        upper[line.length()]=0;
        int ans= (int) 1e9;
        for (int i = 0; i < lower.length; i++) {
            ans= Math.min(ans, upper[i]+lower[i]);
        }
