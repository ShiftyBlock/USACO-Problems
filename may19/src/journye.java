...
public class journye {
    ...
    public static void main(String[] args) throws IOException {
        FastScanner sc = new FastScanner("journye.in", true);
        FastWriter pw = new FastWriter("journye.out", true);
        int N= sc.ni();
        Integer[] arr= new Integer[N];
        HashMap<Integer, Long> dp= new HashMap<>();
        for (int i = 0; i < N; i++) {
            arr[i]= sc.ni();
            if (dp.containsKey(i-arr[i])){
                dp.replace(i-arr[i], dp.get(i-arr[i])+ (long)arr[i]);
            }
            else dp.put(i-arr[i], (long)arr[i]);
        }
        pw.println(Collections.max(dp.values()));
