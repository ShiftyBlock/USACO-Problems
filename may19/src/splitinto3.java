...
public class splitinto3 {
    ...
    public static void main(String[] args) throws IOException {
        FastScanner sc = new FastScanner("splitinto3.in", true);
        FastWriter pw = new FastWriter("splitinto3.out", true);
        int N= sc.ni();
        int a= sc.ni();
        int b= sc.ni();
        int c= sc.ni();
        ...
        dp= new int[N+1];
        Arrays.fill(dp, -1);
        pw.println(solve(N, arr));
        /* End Stub */
        long endTime = System.nanoTime();
        //System.out.println("Execution Time: " + (endTime - startTime) / 1e9 + " s");
        pw.close();
    }
    public static int solve(int len, int[] arr){
        if (0>len) return (int) -1e9;
        if (len==0) return 0;
        if (dp[len]!=-1) return dp[len];
        int res=(int) -1e9;
        for (int i = 0; i < arr.length; i++) {
            res= Math.max(res, 1+ solve(len-arr[i], arr));
        }
        dp[len]=res;
        return res;
    }
