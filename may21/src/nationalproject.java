...
public class nationalproject {
    ...
    public static void main(String[] args) throws IOException {
        FastScanner sc = new FastScanner("nationalproject.in", true);
        FastWriter pw = new FastWriter("nationalproject.out", true);
        int tcs= sc.ni();
        while (tcs-->0){
            int n= sc.ni();
            int a= sc.ni();
            ...
            if (a>=n) System.out.println(n);
                ...
                int need = (int) Math.ceil((double) n / 2.0);
                long total = (long) Math.floor(need / (double) a) * ((long) (a + b));
                if (need % a == 0) total -= b;
                else total += need % a;
                System.out.println(Math.max(n,total));
            }
        }
