...
public class mikenstrings {
    ...
    public static void main(String[] args) throws IOException {
        FastScanner sc = new FastScanner("mikenstrings.in", true);
        FastWriter pw = new FastWriter("mikenstrings.out", true);
        int N= sc.ni();
        HashMap<Integer, HashMap<String, Integer>> cost= new HashMap<>();
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 1; j <line.length() ; j++) {
                ...
                if (!cur.containsKey(temp)) cur.put(temp,j);
                if (i==0){
                    key.put(j, temp);
                }
            }
        ...
        for (int i = 0; i < linelen; i++) {
            String cur= key.get(i);
            int total=0;
            for (int j = 0; j < N; j++) {
                int num=-1;
                if (cost.get(j).containsKey(cur)){
                    num=cost.get(j).get(cur);
                }
                if (num==-1){
                    best=-1;
                    break;
