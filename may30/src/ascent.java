...
public class ascent {
    ...
    public static void main(String[] args) throws IOException {
        FastScanner sc = new FastScanner("ascent.in", true);
        FastWriter pw = new FastWriter("ascent.out", true);
        int n= sc.ni();
        int[] mins= new int[n];
        int[] maxs= new int[n];
        for (int i = 0; i < n; i++) {
        ...
        for (int i = 0; i < n; i++) {
            int a=-1;
            for (int j = n; j >0 ; j/=2) {
                while(n> a+j && mins[i]>= maxs[a+j] ) a+=j;
            }
            bad+=n-a-1;
        }
    ...
    static class Copy implements Comparable<Copy> {
        int num;
        int max;
        boolean canbecopied;
        public Copy(int num, int max, boolean f){
            this.num=num;
            this.max=max;
            this.canbecopied=f;
        }
        public int compareTo(Copy obj){
            if(max==obj.max){
                return num-obj.num;
            }
            return max-obj.max;
