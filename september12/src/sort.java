...
public class sort {
    ...
    public static void main(String[] args) throws Exception {
        boolean debug = !(true);
        FastScanner sc = new FastScanner("sort.in", debug);
        FastWriter pw = new FastWriter("sort.out", debug);
        int N= sc.ni();
        Pair[] arr= new Pair[N];
        for (int i = 0; i < N; i++) {
        ...
        for (int i = 0; i < N; i++) {
            st.update(res[i], st.sum(res[i], res[i]+1)+1);
            ans= Math.max(ans, st.sum(res[i]+1, N));
        }
        pw.println(ans+1);
