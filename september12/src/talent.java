...
public class talent {
    ...
    public static void main(String[] args) throws Exception {
        boolean debug = !(true);
        FastScanner sc = new FastScanner("talent.in", debug);
        FastWriter pw = new FastWriter("talent.out", debug);
        int N= sc.ni();
        int W= sc.ni();
        Pair[] arr= new Pair[N];
        for (int i = 0; i < N; i++) {
            int weight= sc.ni();
            int talent= sc.ni();
            arr[i]= new Pair(weight, talent);
        }
        int[][] dp= new int[N+1][N*1001];
        for (int i = 0; i < N+1; i++) {
            Arrays.fill(dp[i],1<<30);
        }
        dp[0][0]=0;
        for (int i = 1; i <=N; i++) {
            for (int j = 0; j <=N*1001-1; j++) {
                dp[i][j]= dp[i-1][j];
                if(j>=arr[i-1].b) dp[i][j]= Math.min(dp[i][j], dp[i-1][j-arr[i-1].b]+ arr[i-1].a);
            }
        }
        int best=0;
        for (int i = 0; i <=N*1001-1; i++) {
            if(dp[N][i]<W) continue;
            best= Math.max(best, (int) (1000*(double) i/(double)dp[N][i] ));
        }
        pw.println(best);
