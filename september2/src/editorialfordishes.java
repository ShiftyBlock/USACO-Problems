...
public class editorialfordishes {

    static long startTime = System.nanoTime();

    public static void main(String[] args) throws Exception {
        boolean debug = !(true);
        FastScanner sc = new FastScanner("data.in", debug);
        FastWriter pw = new FastWriter("editorialfordishes.out", debug);
        int N= sc.ni();
        int place= 0;
        int ans=N;
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = x; j>0 && base[j]==0; j--) {
                base[j]=x;
            }
            while(!adj[base[x]].isEmpty() && adj[base[x]].get(adj[base[x]].size()-1)< x){
                place= adj[base[x]].get(adj[base[x]].size()-1);
                adj[base[x]].remove(adj[base[x]].size()-1);
            }
            adj[base[x]].add(x);
        }
        pw.println(ans);
