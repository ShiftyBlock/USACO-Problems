...
public class matching {
    ...
    public static void solve() throws Exception{
        boolean debug = (true);
        FastScanner sc = new FastScanner("matching.in", debug);
        FastWriter pw = new FastWriter("matching.out", debug);
        int N= sc.ni();
        adj= new ArrayList[N];
        for(int i=0; i<N; i++){
            adj[i]= new ArrayList<>();
    ...
    static void dfs(int cur, int par){
        ...
        for(int next: adj[cur]){
            if(next==par) continue;
            dp[cur][1]= Math.max(dp[cur][1], dp[next][0]+1+dp[cur][0]-Math.max(dp[next][1], dp[next][0]));
        }
    }
