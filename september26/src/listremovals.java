...
public class listremovals {
    ...
    public static void solve() throws Exception {
        FastScanner sc = new FastScanner("listremovals.in", submit);
        FastWriter pw = new FastWriter("listremovals.out", submit);
        int N= sc.ni();
        int[] arr= new int[N];
        int[] idx= new int[N];
        for (int i = 0; i < N; i++) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            while(low!=high){
                int mid= (low+high)/2;
                int sum= st.sum(0,mid+1);
                if(sum<idx[i] || (taken[mid] && sum==idx[i])) low=mid+1;
                else if(sum>idx[i]) high=mid;
                else{
