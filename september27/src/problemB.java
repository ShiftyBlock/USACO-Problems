...
public class problemB {
    ...
    public static void solve() throws Exception {
        FastScanner sc = new FastScanner("problemB.in", submit);
        FastWriter pw = new FastWriter("problemB.out", submit);
        ...
        while(tcs-->0){
            ...
            for (int i = 0; i < N; i++) {
                int com= K-arr[i];
                if(a.getOrDefault(com, 0) <= b.getOrDefault(com, 0)){
                    a.put(arr[i],a.getOrDefault(arr[i], 0)+1);
                    res[i]=1;
                }
                else{
                    b.put(arr[i],b.getOrDefault(arr[i], 0)+1);
                    res[i]=0;
                }
            }
            for (int i = 0; i < N; i++) {
