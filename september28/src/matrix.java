...
public class matrix {
    ...
    public static void solve() throws Exception {
        FastScanner sc = new FastScanner("matrix.in", submit);
        FastWriter pw = new FastWriter("matrix.out", submit);

        int tcs= sc.ni();
        while(tcs-->0){
            int N= sc.ni(); int M =sc.ni();
            boolean sym= false;
            for (int i = 0; i < N; i++) {
                int a= sc.ni(); int b= sc.ni(); int c= sc.ni(); int d= sc.ni();
                sym|= (b==c);
            }
