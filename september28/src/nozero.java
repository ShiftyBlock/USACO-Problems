...
public class nozero {
    ...
    public static void solve() throws Exception {
        FastScanner sc = new FastScanner("nozero.in", submit);
        FastWriter pw = new FastWriter("nozero.out", submit);

        int N= sc.ni();
        int[] arr= new int[N]; long[] pref= new long[N];
        int res=0;
        ...
        for (int i = 0; i < N; i++) {
            ...
            if(count.containsKey(pref[i]) || pref[i]==0){
                res++; count.clear(); pref[i]=arr[i];
            }
            count.put(pref[i], 1);
