...
public class loan {
    ...
    public static void solve() throws Exception {
        FastScanner sc = new FastScanner("loan.in", submit);
        FastWriter pw = new FastWriter("loan.out", submit);

        long N, K, M;
        N = sc.nl(); K= sc.nl(); M= sc.nl();
    ...
    static boolean valid(long N, long K, long M, long X){
        long reps=0;
        long given=0;
        while(reps<K && given<N){
            long now= (N-given)/X;
            if(now<=M){
                if(given+(M*(K-reps))>=N) return true;
                return false;
            }
            long thing = now * X;
            long extra = 1+ (N - given - thing) / now;
            if(extra+reps>K) extra=K-reps;
            given += extra * now;
            reps += extra;
        }
        return given>=N;
