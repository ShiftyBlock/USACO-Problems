...
public class chefmovr {
    ...
    public static void main(String[] args) throws Exception {
        boolean debug = (true);
        FastScanner sc = new FastScanner("chefmovr.in", debug);
        FastWriter pw = new FastWriter("chefmovr.out", debug);
        int tcs= sc.ni();
        while(tcs-->0){
            int N= sc.ni();
            int D=sc.ni();
            ...
            int moves=0;
            for (int i = 0; i < N-D; i++) {
                long cur= arr[i]; long next= arr[i+D];
                if(cur==next) continue;
            }
            long cur=0;
            boolean valid=true;
            for (int i = 1; i < N; i++) {
                if(arr[i]!=cur) valid=false;
            }
            if(valid)pw.println(moves);
            else pw.println(-1);
        }
