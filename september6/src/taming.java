...
public class taming {
    ...
    public static void main(String[] args) throws Exception {
        boolean debug = !(true);
        FastScanner sc = new FastScanner("taming.in", debug);
        FastWriter pw = new FastWriter("taming.out", debug);
        int N= sc.ni();
        int[] arr= new int[N];
        for (int i = 0; i < N; i++) {
            arr[i]= sc.ni();
        ...
        //dp[day][#of breakout][value of day (breakout or no)]
        ...
        dp[1][1][0]= (arr[0]==0?0:1);
        for (int i = 2; i <=N; i++) {
            for (int j = 1; j <=i; j++) {
                if(j==1) {
                    dp[i][j][i-j]= dp[i-1][j][i-j-1]+ ((i-j)==arr[i-1]?0:1);
                    continue;
                }
                ...
                for (int k = 0; k <=i-j; k++) {
                    if(k==0){
                        int best=1<<30;
                        for (int l = 0; l <=i-j; l++) {
                            best= Math.min(best, dp[i-1][j-1][l]);
                        }
                        dp[i][j][k]=best+(k==arr[i-1]?0:1);
                    }
                    else {
                        dp[i][j][k]= dp[i-1][j][k-1]+(k==arr[i-1]?0:1);
                    }
                }
            }
        }
